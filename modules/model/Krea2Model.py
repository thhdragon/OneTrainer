...
class Krea2Model(BaseModel):
    ...
    def encode_text(
        ...
        if tokens is None and text is not None:
            ...
            suffix = "<|im_end|>\n<|im_start|>assistant\n"
            wrapped_text = [prefix + t for t in text]

            suffix_out = self.tokenizer(suffix, return_tensors="pt")
            suffix_ids = suffix_out.input_ids.to(self.text_encoder.device)
            suffix_mask = suffix_out.attention_mask.to(self.text_encoder.device)

            max_len = PROMPT_MAX_LENGTH + 34 - 5
            ...
            main_ids = inputs.input_ids.to(self.text_encoder.device)
            main_mask = inputs.attention_mask.to(self.text_encoder.device)

            suffix_ids = suffix_ids.repeat(len(text), 1)
            suffix_mask = suffix_mask.repeat(len(text), 1)

            tokens = torch.cat([main_ids, suffix_ids], dim=1)
            tokens_mask = torch.cat([main_mask, suffix_mask], dim=1)
        ...
        if text_encoder_output is None and self.text_encoder is not None:
            with self.text_encoder_autocast_context:
                states = self.text_encoder(
                    input_ids=tokens,
                    attention_mask=tokens_mask.float() if tokens_mask is not None else None,
                ...
                text_encoder_output = hiddens[:, 34:]
                if tokens_mask is not None:
                    tokens_mask = tokens_mask[:, 34:]

        if tokens_mask is None:
            tokens_mask = torch.ones(text_encoder_output.shape[:2], device=text_encoder_output.device)

        seq_lengths = tokens_mask.sum(dim=1)
        max_seq_length = max(1, int(seq_lengths.max().item()))
